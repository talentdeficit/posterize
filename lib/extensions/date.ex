defmodule :posterize_xt_date do
  ...
  def encode(_) do
    quote location: :keep do
      ...
      other ->
        :posterize_xt_date.bad_date(other)
    end
  end
  ...
  def do_encode({year, month, day} = date)
      when year >= @min_year and year < 0 and is_integer(month) and is_integer(day) do
    try do
  ...
  def do_encode({year, month, day} = date)
      when year > 0 and year <= @max_year and is_integer(month) and is_integer(day) do
    try do
