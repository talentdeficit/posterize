defmodule QueryTest do
  ...
  test "decode time + timestamp types", context do
    ...
    units =
      :erlang.convert_time_unit(
        298_771_200_000_000 + 15 * 3_600_000_000 + 3 * 60_000_000 + 48_000_000,
        :micro_seconds,
        :native
      )
  ...
  test "encode intervals", context do
    ...
    assert [[%{months: 1, days: 1, microseconds: 1}]] ==
             query("SELECT $1::interval", [%{months: 1, days: 1, microseconds: 1}])

    assert [[%{months: 12, days: 7, microseconds: 0}]] ==
             query("SELECT $1::interval", [%{years: 1, weeks: 1, microseconds: 0}])

    assert [[%{months: 13, days: 8, microseconds: 3_661_000_000}]] ==
  ...
  test "decode intervals", context do
    ...
    assert [[%{months: 1, days: 1, microseconds: 1}]] ==
             query("SELECT '1 month 1 day 0.000001 seconds'::interval", [])

    assert [[%{months: 12, days: 7, microseconds: 0}]] ==
             query("SELECT '1 year 1 week'::interval", [])

    assert [[%{months: 13, days: 8, microseconds: 3_661_000_000}]] ==
             query("SELECT '1 year 1 month 1 week 1 day 1 hour 1 minute 1 second'::interval", [])
  end
  ...
  test "encode ranges", context do
    ...
    assert [[%{lower: 1, upper: 3, bounds: :"[)"}]] ==
             query("SELECT $1::int4range", [%{lower: 1, upper: 3, bounds: :"[)"}])

    assert [[%{lower: 1, upper: 3, bounds: :"[)"}]] ==
             query("SELECT $1::int4range", [%{lower: 1, upper: 2, bounds: :"[]"}])

    assert [[%{lower: 2, bounds: :"[)"}]] ==
             query("SELECT $1::int4range", [%{lower: 1, bounds: :"()"}])

    assert [[%{upper: 10, bounds: :"()"}]] ==
             query("SELECT $1::int4range", [%{upper: 10, bounds: :"()"}])

    assert [[%{lower: 946_684_800_000_000_000, upper: 978_307_200_000_000_000, bounds: :"[]"}]] ==
             query("SELECT $1::tsrange", [
               %{lower: 946_684_800_000_000_000, upper: 978_307_200_000_000_000, bounds: :"[]"}
             ])
  ...
  test "decode ranges", context do
    ...
    assert [[%{lower: 946_684_800_000_000_000, upper: 978_307_200_000_000_000, bounds: :"[]"}]] ==
             query("SELECT '[2000-01-01,2001-01-01]'::tsrange", [])
  ...
  test "decode json types", context do
    ...
    assert [["[true,1,1.0,\"hallo world\"]"]] ==
             query("SELECT '[true,1,1.0,\"hallo world\"]'::json", [])
  end
  ...
  test "encode json types", context do
    ...
    assert [["[true,1,1.0,\"hallo world\"]"]] ==
             query("SELECT $1::json", ["[true,1,1.0,\"hallo world\"]"])
  end
  ...
  test "decode jsonb types", context do
    ...
    assert [["[true, 1, 1.0, \"hallo world\"]"]] ==
             query("SELECT '[true,1,1.0,\"hallo world\"]'::jsonb", [])
  end
  ...
  test "encode jsonb types", context do
    ...
    assert [["[true, 1, 1.0, \"hallo world\"]"]] ==
             query("SELECT $1::jsonb", ["[true,1,1.0,\"hallo world\"]"])
  end
end
